...
# A function rather than a pipeline at the call site: `run` takes ONE command. pipefail is
# already on from the top of this script and a function body shares the shell's options,
# so setting it here would change them for everything, not just this pipeline.
zstdcat_to_disk() {
